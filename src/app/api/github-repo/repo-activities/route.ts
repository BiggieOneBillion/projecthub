...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const res = await GitHubController.getRepositoryActivities(
    ...
    return NextResponse.json({
      message: "Successfully fetched repo activities",
      data: res,
    });
  } catch (error) {
    console.error("Error fetching repo activities", error);
    return NextResponse.json({
